...
export class CustomerValidator {
  ...
  async validateCreate(data: any): Promise<void> {
    ...
    if (!data.customer_type || typeof data.customer_type !== 'string') {
      throw new BadRequestException('Customer type is required and must be a string');
    }

    if (!this.expectedCustomerTypes.includes(data.customer_type.toLowerCase())) {
      throw new BadRequestException(`Customer type must be one of: ${this.expectedCustomerTypes.join(', ')}`);
    }

    // Validate customer_status
    if (!data.customer_status || typeof data.customer_status !== 'string') {
      throw new BadRequestException('Customer status is required and must be a string');
    }

    if (!this.expectedCustomerStatuses.includes(data.customer_status.toLowerCase())) {
      throw new BadRequestException(`Customer status must be one of: ${this.expectedCustomerStatuses.join(', ')}`);
    }
