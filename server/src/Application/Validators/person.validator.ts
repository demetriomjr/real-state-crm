...
export class PersonValidator {
  ...
  async validateCreate(data: any): Promise<void> {
    ...
    if (!data.full_name || typeof data.full_name !== "string") {
      throw new BadRequestException(
        "Full name is required and must be a string",
      );
    }
    ...
    if (
      !this.expectedDocumentTypes.includes(data.document_type.toLowerCase())
    ) {
      throw new BadRequestException(
        `Document type must be one of: ${this.expectedDocumentTypes.join(", ")}`,
      );
    }
    ...
    if (!data.document_number || typeof data.document_number !== "string") {
      throw new BadRequestException(
        "Document number is required and must be a string",
      );
    }
    ...
    if (data.document_number.length > 20) {
      throw new BadRequestException(
        "Document number must not exceed 20 characters",
      );
    }

    // Validate document number format based on type
    await this.validateDocumentNumberFormat(
      data.document_type,
  ...
  private async validateDocumentNumberFormat(
    documentType: string,
