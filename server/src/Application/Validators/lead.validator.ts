...
export class LeadValidator {
  ...
  async validateCreate(data: any): Promise<void> {
    ...
    if (!data.lead_type || typeof data.lead_type !== "string") {
      throw new BadRequestException(
        "Lead type is required and must be a string",
      );
    }

    if (!this.expectedLeadTypes.includes(data.lead_type.toLowerCase())) {
      throw new BadRequestException(
        `Lead type must be one of: ${this.expectedLeadTypes.join(", ")}`,
      );
    }
    ...
    if (!data.lead_status || typeof data.lead_status !== "string") {
      throw new BadRequestException(
        "Lead status is required and must be a string",
      );
    }

    if (!this.expectedLeadStatuses.includes(data.lead_status.toLowerCase())) {
      throw new BadRequestException(
        `Lead status must be one of: ${this.expectedLeadStatuses.join(", ")}`,
      );
    }
    ...
    if (!data.lead_temperature || typeof data.lead_temperature !== "string") {
      throw new BadRequestException(
        "Lead temperature is required and must be a string",
      );
    }

    if (
      !this.expectedLeadTemperatures.includes(
        data.lead_temperature.toLowerCase(),
      )
    ) {
      throw new BadRequestException(
        `Lead temperature must be one of: ${this.expectedLeadTemperatures.join(", ")}`,
      );
    }
    ...
    if (!data.lead_origin || typeof data.lead_origin !== "string") {
      throw new BadRequestException(
        "Lead origin is required and must be a string",
      );
    }

    if (!this.expectedLeadOrigins.includes(data.lead_origin.toLowerCase())) {
      throw new BadRequestException(
        `Lead origin must be one of: ${this.expectedLeadOrigins.join(", ")}`,
      );
    }
    ...
    if (!data.lead_description || typeof data.lead_description !== "string") {
      throw new BadRequestException(
        "Lead description is required and must be a string",
      );
    }
    ...
    if (data.lead_description.length > 1000) {
      throw new BadRequestException(
        "Lead description must not exceed 1000 characters",
      );
    }
    ...
    if (data.lead_notes !== undefined) {
      ...
      for (let i = 0; i < data.lead_notes.length; i++) {
        ...
        if (typeof note !== "string") {
          throw new BadRequestException(
            `Lead note at index ${i} must be a string`,
          );
        }

        if (!note.trim()) {
          throw new BadRequestException(
            `Lead note at index ${i} cannot be empty`,
          );
        }

        if (note.length > 500) {
          throw new BadRequestException(
            `Lead note at index ${i} must not exceed 500 characters`,
          );
        }
      }
    }
    ...
    if (
      !data.first_contacted_by ||
      typeof data.first_contacted_by !== "string"
    ) {
      throw new BadRequestException(
        "First contacted by is required and must be a string",
      );
    }
  ...
  async validateUpdate(data: any): Promise<void> {
    ...
    if (data.first_contacted_by !== undefined) {
      ...
      if (!uuidRegex.test(data.first_contacted_by)) {
        throw new BadRequestException(
          "First contacted by must be a valid UUID",
        );
      }
    }
