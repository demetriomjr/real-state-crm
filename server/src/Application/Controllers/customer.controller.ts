...
export class CustomerController {
  ...
  @Post()
  async createCustomer(@Body() createCustomerDto: CustomerCreateDto, @Request() req: any): Promise<CustomerResponseDto> {
    ...
    const {
      ...
      addresses, contacts, other_documents
    } = createCustomerDto;
  ...
  ): Promise<{ customers: CustomerResponseDto[]; total: number; page: number; limit: number; totalPages: number }> {
  ...
  @Delete(':id')
  async deleteCustomer(@Param('id') id: string, @Request() req: any): Promise<{ message: string }> {
    const userId = req.user?.id;
