...
export class LeadController {
  ...
  @Post()
  async createLead(@Body() createLeadDto: LeadCreateDto, @Request() req: any): Promise<LeadResponseDto> {
  ...
  ): Promise<{ leads: LeadResponseDto[]; total: number; page: number; limit: number; totalPages: number }> {
  ...
  @Delete(':id')
  async deleteLead(@Param('id') id: string, @Request() req: any): Promise<{ message: string }> {
    const userId = req.user?.id;
