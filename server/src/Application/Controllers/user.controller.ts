...
export class UserController {
  ...
  async findAll(@Query('page') page = 1, @Query('limit') limit = 10, @Request() req: any) {
    ...
    if (userLevel < 8) {
      ...
      throw new BadRequestException('Access denied. Admin level (8+) required to view all users.');
    }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Access denied - Insufficient user level' })
  ...
  async findOne(@Param('id') id: string, @Request() req: any) {
    ...
    if (targetUser && userLevel < targetUser.user_level) {
      throw new BadRequestException('Access denied. Your user level is insufficient to view this user.');
    }
