...
export class AuthorizationController {
  ...
  @ApiResponse({ status: 200, description: 'Login successful', type: AuthorizationResponseDto })
  @ApiResponse({ status: 401, description: 'Invalid credentials' })
  async login(@Body() authRequest: AuthorizationRequestDto): Promise<AuthorizationResponseDto> {
  ...
  @ApiHeader({ name: 'Authorization', description: 'Bearer token', required: true })
  @ApiResponse({ status: 200, description: 'Token refreshed successfully', type: AuthorizationResponseDto })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async refresh(@Headers('authorization') authHeader: string): Promise<AuthorizationResponseDto> {
