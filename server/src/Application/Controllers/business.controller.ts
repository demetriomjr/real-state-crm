...
export class BusinessController {
  ...
  @ApiResponse({ status: 403, description: 'Access denied - Developer level required' })
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Access denied - Admin level required' })
  ...
  async findOne(@Param('id') id: string, @Request() req: any) {
    ...
    if (userLevel < 8) {
      throw new BadRequestException('Access denied. Admin level (8+) required to view business details.');
    }
    
    // Check tenant isolation - users can only access their own business
    if (userTenantId && userTenantId !== id) {
      throw new BadRequestException('Access denied. You can only access your own business.');
    }
  ...
  async create(@Body() createBusinessDto: BusinessCreateDto): Promise<AuthorizationResponseDto> {
  ...
  async update(@Param('id') id: string, @Body() updateBusinessDto: BusinessUpdateDto, @Request() req: any) {
    const userLevel = req.userLevel;
    const userTenantId = req.tenantId;
    
    if (userLevel < 8) {
      throw new BadRequestException('Access denied. Admin level (8+) required to update business.');
    }
    
    // Check tenant isolation - users can only update their own business
    if (userTenantId && userTenantId !== id) {
      throw new BadRequestException('Access denied. You can only update your own business.');
    }
  ...
  @ApiParam({ name: 'id', description: 'Business ID (UUID) - must match tenant_id from request' })
  ...
  async remove(@Param('id') id: string, @Request() req: any) {
    ...
    if (userLevel < 10) {
      throw new BadRequestException('Access denied. Developer level (10) required to delete business.');
    }
    
    // The "id" parameter must match the tenant_id from the request
    if (userTenantId && userTenantId !== id) {
      throw new BadRequestException('Access denied. The business ID must match your tenant ID.');
    }
