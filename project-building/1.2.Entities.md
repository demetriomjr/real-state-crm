...
Objetivo: padronizar o “estado” (status) das principais entidades no MVP e suas transições válidas. O estado é afetado por ações do consultor e por colaterais (ex.: criação/cancelamento de agendamento, primeira mensagem trocada etc.).

Notas gerais
- Terminologia: neste documento, “estado” e “status” são equivalentes; o campo persistido em Lead permanece `status` no P0.
- Auditoria: toda transição registra `updatedAt/updatedBy` e o evento correspondente no histórico do lead.
- Multi-tenant: toda transição valida `tenantId` em ambas as pontas antes de efetivar.

Lead (P0)
- Estados suportados: `"Novo" | "Em contato" | "Agendado" | "Arquivado"`.
- Transições válidas (gatilhos → efeito):
  - Novo → Em contato
    - Gatilhos: primeira mensagem enviada OU recebida via WhatsApp; atualização manual do consultor com nota.
  - Novo → Agendado
    - Gatilhos: criação de Agendamento ativo a partir do lead (mesmo sem troca prévia de mensagens).
  - Em contato → Agendado
    - Gatilhos: criação de Agendamento ativo (Calendar OK).
  - Em contato → Arquivado
    - Gatilhos: decisão manual (sem aderência/sem resposta), exige `deleteReason`.
  - Agendado → Em contato
    - Gatilhos: cancelamento do agendamento (mantém o relacionamento ativo); opcionalmente registrar motivo.
  - Agendado → Arquivado
    - Gatilhos: decisão manual (lead despriorizado após agendamento). Recomendado registrar motivo.
  - Arquivado → Em contato
    - Gatilhos: restauração manual; normalmente acompanhada de nova tentativa de contato.
- Restrições:
  - “Arquivado” não pode ir diretamente para “Agendado” — restaurar para “Em contato” primeiro.
  - Excluir definitivamente está fora do P0; preferir arquivar.

Agendamento (Appointment) (P0)
- Estados: `"Ativo" | "Cancelado"`.
- Derivação: se `canceledAt` definido ⇒ `Cancelado`; caso contrário ⇒ `Ativo`.
- Regras de efeito no Lead:
  - Criar agendamento Ativo ⇒ força Lead a `Agendado`.
  - Cancelar agendamento ⇒ Lead volta a `Em contato` (se estava `Agendado`).

Mensagem (Message) (P0)
- Sem máquina de estados própria no P0; mensagens são imutáveis e compõem o histórico.
- Futuro (P1+): opcional `deliveryStatus` de provedores (enviado/entregue/lido).

Invariantes e validações
- Transições inválidas devem ser bloqueadas com feedback claro ao usuário.
- Todo evento relevante (mensagem, criação/cancelamento de agendamento, arquivamento/restauração) aparece no histórico do lead.
- Nenhuma transição apaga histórico; arquivamento não remove mensagens nem agendamentos já registrados.

---

# 11. Colaterais Técnicos e de Dados (MVP)
